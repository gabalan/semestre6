...
class Model{
  ...
  public function connect(){
    try
    {
      $bdd = new PDO('mysql:host=dbserver;dbname=tsdiallo001;charset=utf8','tsdiallo001','Gabalan11@', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
  ...
  public function send_mail($code,$sujet,$message){
    ...
    if ($donnees=$req->fetch()){
      mail($donnees['mail'],$sujet,$message);
  ...
  **$send: Is the submit field, that will be empty but that we don't verify.*/
  ...
  public function getID(){
   ...
   public function addStudent($name, $surname, $mail, $numero){
    ...
    $iv = 'fuKJU6758gjrufdh';
    $encrypted = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $mail, MCRYPT_MODE_CBC, $iv);
